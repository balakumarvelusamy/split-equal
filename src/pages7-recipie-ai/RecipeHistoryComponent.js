...
const RecipeHistoryComponent = ({ showLatest = false }) => {
  ...
  useEffect(() => {
    const fetchRecipeHistory = async () => {
      try {
        const data = await getData_Any2Column("email", loggedInUserEmail, "type", "recipe-ai");
  ...
  return (
    <div>
      ...
      {showLatest ? (
        ...
        <div className="mb-3">
          <input type="text" className="form-control" placeholder="Search by recipe title..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
        </div>
      )}
      {loading ? (
        <p align="center">
      ...
      <Modal show={showModal} onHide={handleCloseModal} centered>
        <div className="modal-header">
          <h5 className="text-success mb-0">{selectedRecipe && selectedRecipe.title.replace(/"/g, "").trim()}</h5>
              ...
              <div className="d-flex justify-content-start">
                ...
                <div className="p-1">
                  ...
                  <p className="mb-0">
                    <small>
                      {" "}
                      <p className="badge text-dark text-muted px-0 mb-0">{selectedRecipe.date}</p>
                    </small>
                  </p>
                </div>
              </div>
              <p className="mt-1">{formatRecipe(selectedRecipe.recipeText)}</p>
            </div>
          )}
        </Modal.Body>
        <Modal.Footer>
